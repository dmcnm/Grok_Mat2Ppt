%% demo.m — 24_Dropbox_technology__HCCS_PrinterInstall_20180206_r0
...
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'HCCS Network Printer Installation'}, struct("autofit", "none"));
set_shape_box_(sh.item(1), 0.8646, 1.2274, 11.875, 2.6111);
set_shape_box_(sh.item(2), 1.6667, 3.9392, 10.0, 1.8108);
...
r = p.add_run(); r.text = 'February 6, 2018, ';
r = p.add_run(); r.text = 'Rev. 0';
apply_norm_autofit_(tf, struct());

%% ===== Slide 2 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'Overview'}, struct("fontScale", 90000));
body = sh.item(2);
tf = sh.item(2).text_frame();
style_body_levels_(tf, { ...
    'Description of Current Printers', 0; ...
    'Printer Installation Instructions ', 0; ...
    }, struct("listProps", false, "autofit", "none"));
ps = tf.paragraphs();
if numel(ps) >= 2
    p = ps{2}; p.clear_content(); p.level = 0;
    r = p.add_run(); r.text = 'Printer ';
    r = p.add_run(); r.text = 'I';
    r = p.add_run(); r.text = 'nstallation ';
    r = p.add_run(); r.text = 'I';
    r = p.add_run(); r.text = 'nstructions ';
end

%% ===== Slide 3 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'Current Printers'}, struct("fontScale", 90000));
body = sh.item(2);
set_shape_box_(body, 0.2057, 1.3365, 12.7422, 5.4187);
tf = body.text_frame();
style_body_levels_(tf, { ...
    'Teacher’s Lounge', 0; ...
    'Color HP Wireless (Model #): 192.168.1.57', 1; ...
    'HP Color Laserjet 3500: 192.168.1.58', 1; ...
    'Mrs. Teeter’s 1/2nd Classroom', 0; ...
    'BW: HP Laserjet 4050N: IP Address TBD (ideally 192.168.1.59)', 1; ...
    'Outside Mrs. Sanchez Classroom', 0; ...
    'BW: HP Laserjet 4100: 192.168.1.97', 1; ...
    ' ', 0; ...
    }, struct("listProps", false, "autofit", "none"));
ps = tf.paragraphs();
if numel(ps) >= 3
    p = ps{3}; p.clear_content(); p.level = 1;
    r = p.add_run(); r.text = 'HP Color ';
    r = p.add_run(); r.text = 'Laserjet';
    r = p.add_run(); r.text = ' 3500: 192.168.1.58';
end
if numel(ps) >= 4
    p = ps{4}; p.clear_content(); p.level = 0;
    r = p.add_run(); r.text = 'Mrs. Teeter’s 1/2';
    r = p.add_run(); r.text = 'nd';
    r = p.add_run(); r.text = ' Classroom';
end
if numel(ps) >= 5
    p = ps{5}; p.clear_content(); p.level = 1;
    r = p.add_run(); r.text = 'BW: HP ';
    r = p.add_run(); r.text = 'Laserjet';
    r = p.add_run(); r.text = ' 4050N: IP Address TBD (ideally 192.168.1.59)';
end
if numel(ps) >= 7
    p = ps{7}; p.clear_content(); p.level = 1;
    r = p.add_run(); r.text = 'BW: HP ';
    r = p.add_run(); r.text = 'Laserjet';
    r = p.add_run(); r.text = ' 4100: 192.168.1.97';
end

%% ===== Slide 4 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'Installing a Network Printer Using TCP/IP (1/4) '}, struct("fontScale", 90000));
set_shape_box_(sh.item(1), 0.5316, 0.3993, 11.0, 0.7213);
body = sh.item(2);
set_shape_box_(body, 0.2057, 1.3365, 12.7422, 5.4187);
tf = body.text_frame();
style_body_levels_(tf, { ...
    'Overview of steps', 0; ...
    'Control Panel -> Devices and printers', 0; ...
    'Add a printer ', 1; ...
    'Add a Local Printer', 0; ...
    'Create a New Port', 0; ...
    'Select Type of Port: Standard TCP/IP Port', 1; ...
    'Type IP Address in Hostname or IP address: 192.168.1.##', 0; ...
    'Installing the Printer Driver Add Printer -> Have Disk', 0; ...
    'Print Test Page', 0; ...
    }, struct("listProps", false, "autofit", "none"));
ps = tf.paragraphs();
if numel(ps) >= 3
    p = ps{3}; p.clear_content(); p.level = 1;
    r = p.add_run(); r.text = 'Add a printer';
    r = p.add_run(); r.text = ' ';
end
...
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'Installing a Network Printer Using TCP/IP (2/4)'}, struct("fontScale", 90000));
set_shape_box_(sh.item(1), 0.5851, 0.3993, 10.9719, 0.7213);
body = sh.item(2);
set_shape_box_(body, 0.2057, 1.3365, 12.7422, 5.4187);
tf = body.text_frame();
style_body_levels_(tf, { ...
...
sp = sh.add_shape('rect', In(6.7655), In(2.6523), In(0.7083), In(0.2211));
try, sp.fill().background(); catch, end
apply_shape_style_office_(sp);
p = add_pic_(sh, assets, 'image4.png', 5.8205, 1.9459, 6.9882, 4.6414);
if ~isempty(p), p.crop_bottom = 0.21134; end

%% ===== Slide 6 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'Installing a Network Printer Using TCP/IP (3/4) '}, struct("fontScale", 90000));
set_shape_box_(sh.item(1), 0.5851, 0.3993, 11.0351, 0.7213);
body = sh.item(2);
set_shape_box_(body, 0.2057, 1.3365, 12.7422, 5.4187);
tf = body.text_frame();
style_body_levels_(tf, { ...
...
ps = tf.paragraphs();
p = add_pic_(sh, assets, 'image5.png', 2.0007, 2.8412, 5.023, 3.6793);
p = add_pic_(sh, assets, 'image6.png', 7.1518, 2.1811, 5.9242, 4.3394);

%% ===== Slide 7 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'Installing a Network Printer Using TCP/IP (4/4) '}, struct("fontScale", 90000));
set_shape_box_(sh.item(1), 0.5851, 0.3993, 10.9592, 0.7213);
body = sh.item(2);
set_shape_box_(body, 0.2057, 1.3365, 12.7422, 5.4187);
tf = body.text_frame();
style_body_levels_(tf, { ...
...
ps = tf.paragraphs();
sp = sh.add_shape('rect', In(11.5443), In(5.4937), In(1.0759), In(0.3165));
try, sp.fill().background(); catch, end
apply_shape_style_office_(sp);
sp = sh.add_shape('rect', In(3.5232), In(4.3544), In(3.3122), In(0.2324));
try, sp.fill().background(); catch, end
apply_shape_style_office_(sp);
sp = sh.add_shape('rect', In(3.5232), In(4.038), In(3.3122), In(0.2085));
try, sp.fill().background(); catch, end
apply_shape_style_office_(sp);
p = add_pic_(sh, assets, 'image7.png', 1.5888, 2.557, 5.4618, 4.0007);
p = add_pic_(sh, assets, 'image8.png', 7.3755, 2.557, 5.4618, 4.0007);
