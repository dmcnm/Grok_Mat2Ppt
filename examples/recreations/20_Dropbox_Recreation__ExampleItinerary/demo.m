%% demo.m — 20_Dropbox_Recreation__ExampleItinerary
...
sh = s.shapes();
p = add_pic_(sh, assets, 'image1.jpeg', 1.85, -1.1614, 6.25, 9.9061);
if ~isempty(p), p.crop_bottom = 0.31236; end

%% ===== Slide 2 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image1.jpeg', -2.8333, 3.5833, 6.25, 0.4167);
if ~isempty(p), p.crop_bottom = 0.97108; end
p = add_pic_(sh, assets, 'image1.jpeg', -0.2135, 1.4635, 6.25, 4.6562);
if ~isempty(p), p.crop_top = 0.67679; end
