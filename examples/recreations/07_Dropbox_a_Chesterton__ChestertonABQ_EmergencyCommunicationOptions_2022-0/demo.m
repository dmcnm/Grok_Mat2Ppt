%% demo.m — 07_Dropbox_a_Chesterton__ChestertonABQ_EmergencyCommunicationOptions_2022-0
...
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'Chesterton Academy', 'Emergency Communication Options'}, struct("fontScale", 90000));
set_shape_box_(sh.item(1), 1.6667, 1.9557, 10.0, 2.6111);
tb = sh.add_textbox(In(4.1082), In(4.8354), In(4.3143), In(0.9088));
...
r = p.add_run(); r.text = 'February 8, 2022';
apply_run_font_fp_(r, struct("pt", 24));
set_para_align_one_(p, 'ctr');
...
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'Overview of Options'}, struct());
body = sh.item(2);
set_shape_box_(body, 0.5393, 1.2388, 12.5453, 5.5293);
tf = body.text_frame();
style_body_levels_(tf, { ...
    'Comprehensive Email and Texting Programs', 0; ...
    'JupiterEd', 1; ...
    'Potentially cheapest and easiest to set up', 2; ...
    'Flocknotes', 1; ...
    'Much more flexible but also more expensive and time consuming to set up', 2; ...
    'Standalone Texting Solutions', 0; ...
    'ExpertTexting.com', 1; ...
    'Seems straightforward with intuitive user interface', 2; ...
    'Offers two-way texting with parents, if desired', 2; ...
    'Per text pricing makes it more attractive for infrequent, emergency use', 2; ...
    'TextEmAll.com', 1; ...
...
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'JupiterEd.com'}, struct());
body = sh.item(2);
set_shape_box_(body, 0.5393, 1.2388, 11.7207, 5.5293);
tf = body.text_frame();
style_body_levels_(tf, { ...
    'Parents need to login to JupiterEd and opt-in to emergency texting service', 0; ...
    'Another option would be to use non-emergency text messages for weather announcements, but these can get lost as they are common', 1; ...
    }, struct("listProps", false));
ps = tf.paragraphs();
if numel(ps) >= 1
    p = ps{1}; p.clear_content(); p.level = 0;
    r = p.add_run(); r.text = 'Parents need to login to ';
    r = p.add_run(); r.text = 'JupiterEd';
    r = p.add_run(); r.text = ' and opt-in to emergency texting service';
end

%% ===== Slide 4 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'Flocknotes'}, struct("autofit", "none"));
body = sh.item(2);
tf = sh.item(2).text_frame();
style_body_levels_(tf, { ...
    'Comprehensive email and texting service', 0; ...
    'Mrs. Sena is familiar with the service', 0; ...
    'More expensive at ~', 0; ...
...
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'ExpertTexting.com'}, struct("autofit", "none"));
body = sh.item(2);
tf = sh.item(2).text_frame();
style_body_levels_(tf, { ...
    'Com', 0; ...
...
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
style_title_(sh.item(1).text_frame(), {'TextEmAll.com'}, struct("autofit", "none"));
body = sh.item(2);
tf = sh.item(2).text_frame();
style_body_levels_(tf, { ...
    'jkh', 0; ...
