%% demo.m — 22_Dropbox_Sangre_de_Cristo__Aerial_Views_Markup
...
sh = s.shapes();
sp = sh.add_shape('roundRect', In(2.0751), In(4.1041), In(1.3347), In(0.4167));
fill_scheme_(sp, 'accent1', 0.6);
apply_shape_style_office_(sp);
tf = sp.text_frame(); tf.clear(); ps = tf.paragraphs();
p = ps{1}; p.clear_content(); p.level = 0;
r = p.add_run(); r.text = 'b1';
apply_run_font_fp_(r, struct("pt", 12, "latin", 'Symbol', "scheme", 'tx1'));
r = p.add_run(); r.text = ': Coombs’ House';
apply_run_font_fp_(r, struct("pt", 12, "scheme", 'tx1'));
...
sp = sh.add_shape('roundRect', In(6.83), In(3.7708), In(1.3347), In(0.4167));
fill_scheme_(sp, 'accent1', 0.6);
apply_shape_style_office_(sp);
tf = sp.text_frame(); tf.clear(); ps = tf.paragraphs();
p = ps{1}; p.clear_content(); p.level = 0;
r = p.add_run(); r.text = 'b2';
apply_run_font_fp_(r, struct("pt", 12, "latin", 'Symbol', "scheme", 'tx1'));
r = p.add_run(); r.text = ': Smith’s House';
apply_run_font_fp_(r, struct("pt", 12, "scheme", 'tx1'));
set_para_align_one_(p, 'ctr');
apply_no_bullet_all_(tf);
apply_no_autofit_(tf);
set_body_anchor_(tf, 'ctr');
try, tf.word_wrap = true; catch, end
p = add_pic_(sh, assets, 'image1.png', 0.0, 0.75, 10.0, 6.0417);
if ~isempty(p), p.crop_left = 0.19876; end
if ~isempty(p), p.crop_top = 0.16816; end

%% ===== Slide 2 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
sp = sh.add_shape('roundRect', In(2.9167), In(3.26), In(1.3333), In(0.1667));
fill_scheme_(sp, 'accent1', 0.6);
apply_shape_style_office_(sp);
tf = sp.text_frame(); tf.clear(); ps = tf.paragraphs();
p = ps{1}; p.clear_content(); p.level = 0;
r = p.add_run(); r.text = 'a1';
apply_run_font_fp_(r, struct("pt", 9, "latin", 'Symbol', "scheme", 'tx1'));
r = p.add_run(); r.text = ': Coombs’ House';
apply_run_font_fp_(r, struct("pt", 9, "scheme", 'tx1'));
set_para_align_one_(p, 'ctr');
apply_no_bullet_all_(tf);
apply_no_autofit_(tf);
set_body_anchor_(tf, 'ctr');
try, tf.word_wrap = true; catch, end
sp = sh.add_shape('roundRect', In(5.25), In(3.0933), In(1.3333), In(0.1667));
fill_scheme_(sp, 'accent1', 0.6);
apply_shape_style_office_(sp);
tf = sp.text_frame(); tf.clear(); ps = tf.paragraphs();
p = ps{1}; p.clear_content(); p.level = 0;
r = p.add_run(); r.text = 'a2';
apply_run_font_fp_(r, struct("pt", 9, "latin", 'Symbol', "scheme", 'tx1'));
r = p.add_run(); r.text = ': Smith’s House';
apply_run_font_fp_(r, struct("pt", 9, "scheme", 'tx1'));
set_para_align_one_(p, 'ctr');
...
set_body_anchor_(tf, 'ctr');
try, tf.word_wrap = true; catch, end
sp = sh.add_shape('roundRect', In(4.5), In(5.3433), In(1.8333), In(0.1667));
fill_scheme_(sp, 'accent1', 0.6);
apply_shape_style_office_(sp);
tf = sp.text_frame(); tf.clear(); ps = tf.paragraphs();
p = ps{1}; p.clear_content(); p.level = 0;
r = p.add_run(); r.text = 'a3';
apply_run_font_fp_(r, struct("pt", 9, "latin", 'Symbol', "scheme", 'tx1'));
r = p.add_run(); r.text = ': ';
apply_run_font_fp_(r, struct("pt", 9, "scheme", 'tx1'));
r = p.add_run(); r.text = 'Brannock’s';
apply_run_font_fp_(r, struct("pt", 9, "scheme", 'tx1'));
r = p.add_run(); r.text = ' Home Site';
apply_run_font_fp_(r, struct("pt", 9, "scheme", 'tx1'));
set_para_align_one_(p, 'ctr');
apply_no_bullet_all_(tf);
apply_no_autofit_(tf);
set_body_anchor_(tf, 'ctr');
try, tf.word_wrap = true; catch, end
p = add_pic_(sh, assets, 'image2.jpeg', 0.0, 0.75, 10.0, 6.02);

%% ===== Slide 3 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
sp = sh.add_shape('roundRect', In(2.4088), In(4.2708), In(0.4171), In(0.3333));
fill_scheme_(sp, 'accent1', 0.6);
apply_shape_style_office_(sp);
style_textbox_(sp.text_frame(), 'b1', struct("bullets", false, "align", 'ctr', "anchor", 'ctr', "autofit", "none", "pt", 12, "face", 'Symbol', "scheme", 'tx1'));
sp = sh.add_shape('roundRect', In(7.1638), In(3.7708), In(0.5005), In(0.3333));
fill_scheme_(sp, 'accent1', 0.6);
apply_shape_style_office_(sp);
style_textbox_(sp.text_frame(), 'b2', struct("bullets", false, "align", 'ctr', "anchor", 'ctr', "autofit", "none", "pt", 12, "face", 'Symbol', "scheme", 'tx1'));
p = add_pic_(sh, assets, 'image1.png', 0.0, 0.75, 10.0, 6.0417);
if ~isempty(p), p.crop_left = 0.19876; end
if ~isempty(p), p.crop_top = 0.16816; end
...
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
sp = sh.add_shape('roundRect', In(4.1667), In(2.6767), In(0.4167), In(0.17));
fill_scheme_(sp, 'accent1', 0.6);
apply_shape_style_office_(sp);
style_textbox_(sp.text_frame(), 'a1', struct("bullets", false, "align", 'ctr', "anchor", 'ctr', "autofit", "none", "pt", 9, "face", 'Symbol', "scheme", 'tx1'));
sp = sh.add_shape('roundRect', In(5.3333), In(3.0933), In(0.4167), In(0.1667));
fill_scheme_(sp, 'accent1', 0.6);
apply_shape_style_office_(sp);
style_textbox_(sp.text_frame(), 'a2', struct("bullets", false, "align", 'ctr', "anchor", 'ctr', "autofit", "none", "pt", 9, "face", 'Symbol', "scheme", 'tx1'));
sp = sh.add_shape('roundRect', In(4.4167), In(5.4267), In(0.4167), In(0.1667));
fill_scheme_(sp, 'accent1', 0.6);
apply_shape_style_office_(sp);
style_textbox_(sp.text_frame(), 'a3', struct("bullets", false, "align", 'ctr', "anchor", 'ctr', "autofit", "none", "pt", 9, "face", 'Symbol', "scheme", 'tx1'));
p = add_pic_(sh, assets, 'image2.jpeg', 0.0, 0.75, 10.0, 6.02);
