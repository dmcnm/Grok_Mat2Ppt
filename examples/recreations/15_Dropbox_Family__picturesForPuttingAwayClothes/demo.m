%% demo.m — 15_Dropbox_Family__picturesForPuttingAwayClothes
...
sh = s.shapes();
p = add_pic_(sh, assets, 'image1.jpg', 4.4632, 0.3512, 3.0946, 2.9969);
p = add_pic_(sh, assets, 'image2.jpg', 0.5504, 4.1519, 3.6646, 3.0786);
if ~isempty(p), p.crop_bottom = 0.08148; end
p = add_pic_(sh, assets, 'image3.jpg', 8.1844, 3.7107, 2.193, 3.5198);
p = add_pic_(sh, assets, 'image4.jpg', 10.9275, 3.6888, 2.0852, 3.5417);
p = add_pic_(sh, assets, 'image1.jpg', 0.7419, 0.3512, 3.0946, 2.9969);
p = add_pic_(sh, assets, 'image5.png', 8.1844, 0.2492, 3.709, 2.9969);
p = add_pic_(sh, assets, 'image6.jpg', 4.5508, 4.4297, 3.0069, 2.8764);
if ~isempty(p), p.crop_bottom = 0.06314; end

%% ===== Slide 2 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
sp = sh.add_shape('roundRect', In(6.0482), In(2.4668), In(2.5408), In(2.2687));
try, sp.fill().background(); catch, end
apply_shape_style_office_(sp);
sp = sh.add_shape('roundRect', In(9.6345), In(0.3601), In(3.4369), In(2.1067));
try, sp.fill().background(); catch, end
apply_shape_style_office_(sp);
p = add_pic_(sh, assets, 'image7.png', 0.4538, 0.3601, 3.2349, 3.2349);
p = add_pic_(sh, assets, 'image8.jpg', 8.1717, 4.8957, 2.4245, 2.7841);
if ~isempty(p), p.crop_right = 0.10577; end
p = add_pic_(sh, assets, 'image9.gif', 5.4441, 0.3601, 3.6496, 4.3754);
if ~isempty(p), p.crop_top = 0.10343; end
if ~isempty(p), p.crop_bottom = 0.09782; end
p = add_pic_(sh, assets, 'image9.gif', 9.5319, 0.3601, 3.6496, 4.3754);
if ~isempty(p), p.crop_top = 0.10343; end
if ~isempty(p), p.crop_bottom = 0.09782; end
p = add_pic_(sh, assets, 'image10.jpg', 0.6432, 3.5236, 3.6496, 3.4508);
if ~isempty(p), p.crop_right = 0.50317; end
if ~isempty(p), p.crop_bottom = 0.14066; end
