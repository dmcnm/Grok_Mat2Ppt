%% demo.m — 10_Dropbox_aDietrich__Oregon State Report Pictures
...
sh = s.shapes();
p = add_pic_(sh, assets, 'image1.jpg', 0.3121, 0.0, 8.4986, 7.5);

%% ===== Slide 2 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image2.jpg', 3.4046, -1.0732, 7.5, 9.6463);

%% ===== Slide 3 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image3.gif', 3.5802, 0.0, 7.4198, 7.5);

%% ===== Slide 4 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image4.jpg', 0.1957, 0.0037, 9.9951, 7.4963);

%% ===== Slide 5 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image5.jpg', 1.3684, 0.0, 10.5471, 7.5353);

%% ===== Slide 6 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image6.png', 0.4167, 0.0, 12.5, 7.5);

%% ===== Slide 7 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image8.jpg', 1.6667, 0.0, 10.0, 7.5);

%% ===== Slide 8 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image9.jpg', 0.945, 0.0008, 11.0459, 7.4992);

%% ===== Slide 9 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image10.jpg', -0.0174, 0.0, 13.3681, 7.5);

%% ===== Slide 10 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image11.jpg', 2.4267, -1.0781, 7.5, 9.6563);

%% ===== Slide 11 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image12.png', 1.1358, 0.027, 7.3583, 7.3583);

%% ===== Slide 12 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image13.jpeg', 2.7895, -1.0005, 7.4908, 9.5102);

%% ===== Slide 13 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
p = add_pic_(sh, assets, 'image14.png', 1.9806, -1.146, 7.5, 9.792);

%% ===== Slide 14 · Title and Content =====
s = prs.slides().add_slide(lay_Title_and_Content);
sh = s.shapes();
tb = sh.add_textbox(In(1.4188), In(1.647), In(1.8257), In(0.4039));
style_textbox_(tb.text_frame(), 'Mild and moist', struct("bullets", false, "autofit", "shape", "bold", true, "rgb", [0 112 192]));
tb = sh.add_textbox(In(6.1714), In(3.628), In(2.3515), In(0.5722));
style_textbox_(tb.text_frame(), 'High Desert', struct("bullets", false, "autofit", "shape", "pt", 28, "bold", true, "rgb", [0 112 192]));
p = add_pic_(sh, assets, 'image15.jpg', 0.1674, 0.0, 10.4839, 7.5);
