%% demo.m — 06_Box_Zeffy__Donation_Form_Question_Discrepencies_by_Link_Type_
...
sh = s.shapes();
tb = sh.add_textbox(In(1.7445), In(0.6874), In(1.6979), In(1.3127));
tf = tb.text_frame(); tf.clear(); ps = tf.paragraphs();
p = ps{1}; p.clear_content(); p.level = 0;
r = p.add_run(); r.text = 'Anonymous Donation Check Box';
apply_run_font_fp_(r, struct("bold", true, "rgb", [0 176 80]));
p = tf.add_paragraph(); p.clear_content(); p.level = 0;
r = p.add_run(); r.text = 'Present';
apply_run_font_fp_(r, struct("bold", true, "rgb", [0 176 80]));
apply_no_bullet_all_(tf);
tf.auto_size = mat2ppt.enum.MSO_AUTO_SIZE.SHAPE_TO_FIT_TEXT;
try, tf.word_wrap = true; catch, end
tb = sh.add_textbox(In(1.7188), In(5.1775), In(1.6979), In(1.3127));
tf = tb.text_frame(); tf.clear(); ps = tf.paragraphs();
p = ps{1}; p.clear_content(); p.level = 0;
r = p.add_run(); r.text = 'Anonymous Donation Check Box';
apply_run_font_fp_(r, struct("bold", true, "rgb", [0 176 80]));
...
tf.auto_size = mat2ppt.enum.MSO_AUTO_SIZE.SHAPE_TO_FIT_TEXT;
try, tf.word_wrap = true; catch, end
tb = sh.add_textbox(In(1.7187), In(3.0118), In(1.6979), In(1.3127));
style_textbox_(tb.text_frame(), 'Anonymous Donation Check Box Missing', struct("bullets", false, "autofit", "shape", "bold", true, "rgb", [255 192 0]));
sp = sh.add_shape('leftBrace', In(3.1146), In(2.8542), In(0.3021), In(1.5729));
sp = sh.add_shape('leftBrace', In(3.1403), In(0.5573), In(0.3021), In(1.5729));
sp = sh.add_shape('leftBrace', In(3.1556), In(4.6997), In(0.3021), In(2.243));
p = add_pic_(sh, assets, 'image1.png', 3.4167, 0.5, 6.5, 6.5);

%% ===== Slide 2 · Blank =====
s = prs.slides().add_slide(lay_Blank);
sh = s.shapes();
tb = sh.add_textbox(In(1.7549), In(0.6375), In(1.6979), In(1.3127));
tf = tb.text_frame(); tf.clear(); ps = tf.paragraphs();
p = ps{1}; p.clear_content(); p.level = 0;
r = p.add_run(); r.text = 'Anonymous Donation Check Box';
apply_run_font_fp_(r, struct("bold", true, "rgb", [0 176 80]));
...
tf.auto_size = mat2ppt.enum.MSO_AUTO_SIZE.SHAPE_TO_FIT_TEXT;
try, tf.word_wrap = true; catch, end
sp = sh.add_shape('leftBrace', In(3.0361), In(0.6062), In(0.4466), In(1.35));
sp = sh.add_shape('rect', In(3.4528), In(0.6375), In(3.6201), In(0.3208));
try, sp.fill().background(); catch, end
apply_shape_style_office_(sp);
tb = sh.add_textbox(In(2.2236), In(3.3297), In(1.6979), In(1.3127));
style_textbox_(tb.text_frame(), 'Anonymous Donation Check Box Missing', struct("bullets", false, "autofit", "shape", "bold", true, "rgb", [255 192 0]));
sp = sh.add_shape('leftBrace', In(3.4827), In(3.3973), In(0.418), In(1.1826));
p = add_pic_(sh, assets, 'image2.png', 3.3508, 0.6687, 4.975, 1.35);
p = add_pic_(sh, assets, 'image3.png', 3.5869, 3.386, 4.7389, 1.35);
if ~isempty(p), p.crop_top = 0.03311; end
if ~isempty(p), p.crop_right = 0.01631; end
